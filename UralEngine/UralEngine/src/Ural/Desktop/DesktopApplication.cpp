...
#define BIND_EVENT_FN(x) std::bind(&DesktopApplication::x, this, std::placeholders::_1)
...
//         glShaders();
    ...
    void DesktopApplication::OnEvent(Event& e)
    {
    ...
    bool DesktopApplication::OnWindowResize(WindowResizeEvent &e)
    {
        int w = e.GetWidth();
        int h = e.GetHeight();
        glViewport(0, 0, w, h);
    ...
    bool DesktopApplication::OnWindowClose(WindowCloseEvent &e)
    {
        m_Running = false;
        return true;
    }
...
//                layer->OnUpdate();
            ...
            std::pair<float, float> position = DesktopInput::GetMousePosition();
    ...
    void DesktopApplication::PushLayer(Layer *layer)
    {
        m_LayerStack.PushLayer(layer);
    }

    void DesktopApplication::PushOverlay(Layer *layer)
    {
